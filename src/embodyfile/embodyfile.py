...
def data2ecg(dataset: Data, fname: Path) -> None:
    ...
    if not data:
        return
    logging.info(f"Converting to ECG file: {fname}")
    sorted_data: list[tuple[int, file_codec.PulseRawList]] = sorted(
        data, key=itemgetter(0)
    )
    columns = ["Raw", "LeadsOn"]
    column_data = []
